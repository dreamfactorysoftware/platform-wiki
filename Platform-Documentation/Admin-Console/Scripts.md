...
There are two types of scripts: 1) event scripts and 2) custom scripts. You should familiarize yourself with some scripting examples and blog posts [here](Server-Side-Scripting). 

Event scripts are like web hooks. You can add a script to any API call, to both pre-process information going to the server in an API request and to post-process information coming from the server in an API response. 

Custom scripts enable you to implement any logic as a web service API that you can call from your application.
