...
DreamFactory trackers fire _events_, which are `GET` requests of a [DreamFactory collector](collectors), whenever an event on a website or application takes place. By appending parameters and values to the end of those `GET`  requests, trackers can pass data into the collectors, for processing by DreamFactory.

The DreamFactory Tracker Protocol is the list of all the parameters that DreamFactory trackers use when firing events to push data into the [DreamFactory collectors] (collectors). Each parameter maps onto one or more fields in the [DreamFactory events table] (canonical-event-model) employed in storage. Here we document which field in the [DreamFactory events table] (canonical-event-model) each parameter added to the query string maps onto.

DreamFactory has been architected to be as easy as possible for developers to create their own alternative subsystems. This documentation should be used by anyone who would like to build their own tracker: by utilising the parameters documented here, the author of a new tracker can be confident that his / her tracker will work with the rest of the DreamFactory stack, and be confident where the values associated with each parameter on every call will be available to query in DreamFactory, whether that's in Hive or Infobright or another database.
...
In the [first part of this guide](#common), we cover the parameters in the DreamFactory tracker protocol that are common across different event types. [In the second part](#events), we document the parameters that are relevant for specific events that are recognised in the DreamFactory event model. Please note: this model is evolving over time as we incorporate more events and grow the set of fields associated with each of the standard events. In all cases, we do our best to ensure that any changes are backwards compatible. (So we are happy adding new parameters, but do not remove parameters once they have been incorporated.)

[Back to top](#top)


## The DreamFactory Tracker protocol: individual parameters
...
- 1. [Common parameters (platform and event independent)](#common)
  ...
  - 1.4 [DreamFactory tracker version](#version)
...
As a DreamFactory user, you can define application IDs for each of your different ditial products and track behaviour of your users across all of them using the same DreamFactory instance by setting the `app_id` in your tracker of choice.
...
Every line of data passed from the tracker should contain an event field (`e`) to denote the type of event being tracked. For details about the potential values that `e` can take, and the corresponding event types that they refer to, see the section detailing [DreamFactory events](#events).
...
<a name="version" />
#### 1.4 DreamFactory Tracker Version

| **Parameter** | **Maps to**      | **Type** | **Description**               | **Implemented?** | **Example values**        |
|:--------------|:-----------------|:---------|:------------------------------|:-----------------|:--------------------------|
| `tv`          | `v_tracker`      | text     | Identifier for DreamFactory tracker | No             | `js-0.5.1`                |
...
Setting `ip` is optional: DreamFactory collectors log IP address as standard. However, you can override the value derived from the collector by populating this value in the tracker. This is useful, if traffic is being proxied to a DreamFactory collector, for example.
...
<a name="events" />
### 3. DreamFactory events

At its heart, DreamFactory is a platform for granular tracking of events. Currently, DreamFactory understands the following events. In the tracker protocol, each event is denoted by an `e=...` parameter.
...
We are working to make the data model for each of the above events richer, and expand the 'DreamFactory event library' to support a wider selection of events that businesses running DreamFactory wish to track.

In each case, we use the `&e` parameter to indicate the type of event that is being tracked by DreamFactory to the value indicated in the above table
...
Note: if possible, it often makes sense to pass in the `user_id` generated by your ad server into the `ad_uid` field, so that this can be matched with the `user_id` generated by DreamFactory at analysis time.
...
Custom event tracking is used to track events that are not natively supported by DreamFactory. (Like ad impressions, page views, ecomm transactions.)
...
Custom unstructured event tracking is used to track events that are not natively supported by DreamFactory and allow arbitrary name: value pairs associated with the event.
