...
We recommend self-hosting the DreamFactory tracking JavaScript, `dreamfactory.js` as it does have some definite advantages over using a third-party-hosted JavaScript:
...
For the purposes of this guide, we are going to assume that you want to serve the standard `dreamfactory.js` from CloudFront. (We discuss other approaches in the [Advanced options](#advanced-options) section below.). To accomplish this, you will need the following:
...
First create a new bucket within your Amazon S3 account to store the pixel. Call this bucket something like `dreamfactory-static-js`:
...
* You won't be able to call this bucket exactly `dreamfactory-static-js`. This is because Amazon S3 bucket names have to be globally unique, and `dreamfactory-static-js` is unfortunately taken by us!

### 2. Upload the JavaScript

You want to upload the **minified** version of the DreamFactory JavaScript, which is called `sp.js`. You can obtain the latest version of the Javascript from the [[hosted assets]] section or review the [DreamFactory Github repo](https://github.com/dreamfactory/dreamfactory).
...
The only disadvantage of a long expiration is that you need to find a way to *force* end users to fetch a fresh copy of `sp.js` when you upgrade to a newer version. This is easily managed by saving your new version to a new folder in your S3 bucket, and updating your DreamFactory tags to point to the new version.
...
For the **Origin Domain Name**, choose your S3 bucket (`dreamfactory-static-js`) from the dropdown, and accept the suggested **Origin ID**. Now hit **Continue**:
...
Write down your CloudFront distribution's **Domain Name** - e.g. `http://d1fc8wv8zag5ca.cloudfront.net`. You will need this later when you integrate DreamFactory into your website.
...
That's it - you now have a CloudFront distribution which will serve your DreamFactory JavaScript to anybody anywhere in the world, fast. Now all that remains is to update your DreamFactory header tag to fetch your own version of `sp.js`, rather than the version hosted by the DreamFactory team.

### 5. Update your tracking tags to use the self-hosted version of `DreamFactory.js`

The standard DreamFactory tracking tag looks something like:
...
The reference to `'://d1fc8wv8zag5ca.cloudfront.net/0.13.1/sp.js'` loads `sp.js`, the DreamFactory JavaScript tracker. The version loaded is the version [hosted by the DreamFactory team from our own Cloudfront subdomain](hosted-assets) (and provided free to the community).
...
* Update the contents of `dreamfactory.js` and then minify it yourself, _and/or:_
* Add the un-minified `dreamfactory.js` into your own asset pipelining process and serve it using something other than CloudFront

The first option above is explored in more detail in the guide to [[Modifying dreamfactory.js|Modifying-dreamfactory-js]].

The second option is out of the scope of the DreamFactory documentation but you should get some ideas as to how the minification should be handled from that same guide, [[Modifying dreamfactory.js|Modifying-dreamfactory-js]].
