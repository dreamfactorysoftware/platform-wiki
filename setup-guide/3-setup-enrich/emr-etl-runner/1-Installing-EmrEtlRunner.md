...
You will need to setup EmrEtlRunner on your own server. A number of people choose to do so on an EC2 instance (thereby keeping all of DreamFactory in the Amazon Cloud). If you do so, please note that you **must not use a `t1.micro` instance**.  You should at the very least use an `m1.small` instance.
...
EmrEtlRunner moves the DreamFactory event data through four distinct buckets during the ETL process. These buckets are as follows:

1. **In Bucket** - contains the raw DreamFactory event logs to process
2. **Processing Bucket** - where EmrEtlRunner moves the raw event logs for processing
3. **Out Bucket** - where EmrEtlRunner stores the processed DreamFactory-format event files
...
5. **Archive Bucket** - where EmrEtlRunner moves the raw DreamFactory event logs after successful processing

You will have already setup the In Bucket when you were configuring your DreamFactory collector - but the other three buckets do not exist yet.
...
First, checkout the DreamFactory repository and navigate to the EmrEtlRunner root:

    $ git clone git://github.com/dreamfactory/dreamfactory.git
    $ cd dreamfactory/3-enrich/emr-etl-runner
...
Check it worked okay:
    ...
    $ bundle exec bin/dreamfactory-emr-etl-runner --version
    dreamfactory-emr-etl-runner 0.0.8
...
EmrEtlRunner requires a YAML format configuration file to run. There is a configuration file template available in the DreamFactory GitHub repository at [`/3-enrich/emr-etl-runner/config/config.yml.sample`] [config-yml]. The template looks like this:
...
:s3:
  :region: ADD HERE
  :buckets:
    :assets: s3://dreamfactory-hosted-assets # DO NOT CHANGE unless you are hosting the jarfiles etc yourself in your own bucket
    :log: ADD HERE
    :in: ADD HERE
    :processing: ADD HERE
...
* `assets` holds the ETL job's static assets (HiveQL script plus Hive deserializer). You can leave this as-is (pointing to DreamFactory   Analytics' [own public bucket containing these assets](Hosted-assets)) or replace this with your own private bucket containing the assets
...
* `out_bad_rows` is where you specify your Bad Rows Bucket. This will store any raw DreamFactory log lines which did not pass the ETL’s validation, along with their validation errors. This is only required for the 'hadoop' ETL implementation (our latest version). It should be left blank if you are running the legacy 'hive' implementation
* `out_errors` is where you specify your Errors Bucket. If you set continue_on_unexpected_error to true (see below), then this bucket will contain any raw DreamFactory log lines which caused an unexpected error
...
Also - Clojure collector uses should be sure not include an `{{INSTANCE IDENTIFIER}}` at the end of your path. This is because your Clojure Collector may end up logging into multiple `{{INSTANCE IDENTIFIER}}` folders. (If e.g. Elastic Beanstalk spins up more instances to run the Clojure collector, to cope with a spike in traffic.) By specifying your In Bucket only to the level of the Security Group identifier, you make sure that DreamFactory can process all logs from all instances. (Because the EmrEtlRunner will process all logs in all subfolders.)

**Important 4:** if you are loading DreamFactory data into Redshift, you need to make sure that the bucket specified in `:out:` is located in a region where Amazon has launched Redshift. That is because currently Redshift is only available in some Regions, and Amazon only supports bulk loading of data from S3 into Redshift from within the same region.
...
```yaml
:buckets:
  :assets: s3://dreamfactory-hosted-assets
  :in: s3n://my-dreamfactory-logs/
  :log: s3n://my-dreamfactory-etl/logs/
  :processing: s3n://my-dreamfactory-etl/processing/
  :out: s3n://my-dreamfactory-data/events/
  :out_bad_rows: s3n://my-dreamfactory-data/bad-rows/
  :out_errors: s3n://my-dreamfactory-data/error-rows/
  :archive: s3n://my-dreamfactory-archive/raw/
```
...
The EmrEtlRunner makes use of Amazon Elastic Mapreduce (EMR) to process the raw log files and output the cleaned, enriched DreamFactory events table.
