...
Note: In most cases the subfolders present in the above folder will be of the form `part-0000...`, if you are running the 'hadoop' version of the ETL process to output the data into Redshift.

If alternatively you are running the legacy 'hive' ETL (because e.g. you want to use Hive or Infobright as your storage target, rather than Redshift, which is the only storage target the 'hadoop' etl currently supports), the subfolders names will be of the format `dt=...`.
